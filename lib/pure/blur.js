...
function convolveFilter(
  ...
  for (i = 0; i < size; i++ ) {
    ...
    bufferTo[locationIndex] = sumCache[valueSum];
...
function filterHorizontally(filter) {
  ...
  for (y = 0; y < filter.height; y++ ) {
    // Compute the sums
    sums = computeWindowSums(filter.radius, locationIndex, filter.buffer, filter.stack, widthMinus1);
...
function filterVertically(filter) {
  ...
  for (x = 0; x < filter.width; x++) {
    ...
    sums = computeWindowSums(
            filter.radius, locationIndex, filter.values, filter.stack,
            heightMinus1, filter.width, -filter.radius*filter.width);

    // Compute the concrete values for the current row
    // move values from the filter.values to filter.buffer
    convolveFilter(
      filter.height, filter.width, filter.divisionCache, filter.buffer, filter.values,
...
function fastblur(src, width, height, radius, steps) {
  ...
  // 256 for 8bit array
  // If we wanted to precalculate an array for
  // 32bit values, we'd have to calculate
  // if the radius is 4, we'd have
  // new Uint32Array(2^32 * 10)
  // new Uint32Array(42949672960)
  // It's around 160gb of memory
  //
  var maxDivision = 256 * divsum | 0;
  ...
  var filter = {
    ...
    width: width,
    height: height,
    divisionCache: new Uint8Array(maxDivision)
  };
